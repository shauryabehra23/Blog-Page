...
export default function ReviewSectionModal({
   ...
   * Purpose: Displays and allows author to edit the approved live content
   ...
   * Purpose: Displays the collaborator's draft and allows author to tweak it
   * Initialize with: section.draftContent
   * On Approve: Right editor content → both approvedContent and draftContent
   * On Reject: Right editor content → draftContent (author's fixes sent back to collaborator)
   */
   ...
   * Workflow:
   * 1. Grab current JSON/HTML from RIGHT Editor (in case author made final tweaks)
   * 2. Send PUT with: { approvedContent: rightEditorContent, draftContent: rightEditorContent, status: 'approved', feedback: '' }
   * 3. Logic: Makes draft live AND syncs draft state so collaborator's next edit starts from approved version
   */
  const handleApprove = async () => {
    try {
      ...
      // Grab current content from RIGHT editor (may contain author's tweaks)
   ...
   * Workflow:
   * 1. Grab current JSON/HTML from RIGHT Editor (contains author's fixed content)
   * 2. Grab text from authorFeedback textarea
   * 3. Send PUT with: { draftContent: rightEditorContent, status: 'rejected', feedback: authorFeedback }
   * 4. Logic: Do NOT touch approvedContent. By saving author's tweaks to draftContent,
   *    collaborator will fetch these exact edits next time they open editor
   */
  const handleReject = async () => {
    try {
      ...
      // Grab current content from RIGHT editor (contains author's fixes)
      const rightEditorContent = draftEditor?.getJSON();
    ...
    <div className="review-modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="review-modal-body two-way-body">
          {/* LEFT: Master Version (Always Visible, Always Editable) */}
          ...
          {/* RIGHT: Collaborator Draft (Always Visible, Always Editable by Author) */}
        ...
        <div className="review-modal-actions">
          ...
          <div className="action-buttons">
            <button
              ...
              disabled={isSubmitting}
              title="Approve: Saves right editor content to both approvedContent and draftContent, clears feedback"
            >
            ...
            <button
              ...
              disabled={isSubmitting}
              title="Reject: Saves right editor content to draftContent with feedback, author's tweaks become the new draft for collaborator"
            >
